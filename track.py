...
def detect(opt):
    ...
    attempt_download(
        deep_sort_weights, repo="mikel-brostrom/Yolov5_DeepSort_Pytorch"
    )
    deepsort = DeepSort(
    ...
    for frame_idx, (path, img, im0s, vid_cap) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        # count the trackable object
                        # to move to utils or track or somehwere
                        track = output[6]
                        if not track.counted:
                            x = [c[0] for c in track.centroids]
                            direction = track.mean[0] - np.mean(x[-5:])
                            if (
                                direction * counting_direction > 0
                                and W_line1 < track.mean[0] < W_line2
                            ):
                                totalUp += 1
                                track.counted = True
                                detection_frame_list.append(frame_idx)
            ...
            # print counting results to frame
            plot_counting(
                W_line1,
                W_line2,
                totalUp,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--evaluate", action="store_true", help="augmented inference"
    )
    parser.add_argument(
        "--config_deepsort",
        type=str,
    ...
    parser.add_argument(
        "--W_line1", type=int, default=350, help="first line for counting"
    )
    parser.add_argument(
        "--W_line2", type=int, default=900, help="second line for counting"
    )
    parser.add_argument(
        "--counting_direction", type=int, default=1, help="1 is left to right"
    )
